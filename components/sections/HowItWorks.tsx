      ...
      <ol className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {steps.map(({ icon: Icon, title, text }, i) => (
          <Reveal key={title} delay={i * 0.08}>
            <li className="relative h-full rounded-2xl bg-white p-6 pt-8 shadow-soft ring-1 ring-ink-100">
              <span className="absolute -top-4 left-6 flex size-8 items-center justify-center rounded-full bg-brand-600 text-sm font-bold text-white shadow-md">
                {i + 1}
              </span>
              <span className="mb-4 inline-flex size-11 items-center justify-center rounded-xl bg-brand-50 text-brand-700 ring-1 ring-brand-100">
